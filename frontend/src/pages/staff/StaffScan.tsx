...
import { CheckCircle, XCircle, Camera, RefreshCw, User, Calendar, MapPin, Hash } from "lucide-react";
...
export default function StaffScan() {
  ...
  // Refs prevent stale closures in the scanner callback and track instances
  const isVerifyingRef = useRef(false);
  const scannerRef = useRef<Html5QrcodeScanner | null>(null);

  useEffect(() => {
    // Strict Mode double-render fix: don't initialize if already present
    const readerNode = document.getElementById("reader");
    if (readerNode && readerNode.innerHTML !== "") return;
    ...
    scanner.render(
      (decodedText) => {
        ...
        // Pause scanner to prevent continuous scanning spam
        try {
  ...
  const handleVerify = async (bookingId: number) => {
    ...
    try {
      const response = await apiPost<any>("/bookings/verify-qr", { booking_id: bookingId });
      if (response.success) {
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4">
      <div className="max-w-md mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500">Dành cho nhân viên xác minh vé khách hàng</p>
        </div>

        {/* Scanner Area (Keep in DOM, just hide when there's a result) */}
        <div className={scanResult || error ? "hidden" : "block"}>
          <div className="bg-white p-4 rounded-3xl shadow-xl overflow-hidden border border-slate-100">
            ...
            <div className="mt-4 p-4 bg-blue-50 rounded-xl flex items-start gap-3">
              ...
              <p className="text-xs text-blue-700 leading-relaxed">
                Đang bật camera... Vui lòng đưa mã QR của khách hàng vào khung hình để tự động nhận dạng.
              </p>
            </div>
          </div>
        ...
        {scanResult && (
          <div className="bg-white rounded-3xl shadow-2xl p-8 border-t-8 border-green-500 animate-in fade-in zoom-in duration-300">
            ...
            <h2 className="text-2xl font-bold text-center text-slate-900 mb-2">Hợp Lệ!</h2>
            <p className="text-center text-green-600 font-medium mb-8">Check-in thành công</p>

            <div className="space-y-4 border-t border-slate-100 pt-6">
              <div className="flex items-center gap-4 p-3 bg-slate-50 rounded-xl">
                <User className="w-5 h-5 text-slate-400" />
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-slate-400 font-bold">Khách hàng</p>
                  <p className="font-bold text-slate-800">{scanResult.customer_name}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 p-3 bg-slate-50 rounded-xl">
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-slate-400 font-bold">Dịch vụ</p>
                  <p className="font-bold text-slate-800">{scanResult.service_name}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 p-3 bg-slate-50 rounded-xl">
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-slate-400 font-bold">Thời gian quét</p>
                  <p className="font-bold text-slate-800">{scanResult.checked_in_at}</p>
                </div>
              </div>
            </div>
        ...
        {error && (
          <div className="bg-white rounded-3xl shadow-2xl p-8 border-t-8 border-red-500 animate-in fade-in zoom-in duration-300">
            ...
            <h2 className="text-2xl font-bold text-center text-slate-900 mb-2">Thất Bại</h2>
