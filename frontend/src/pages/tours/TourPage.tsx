    ...
    if (searchInput.trim() !== "") {
      ...
      result = result.filter(
        (t) =>
          ...
          t.departure_location?.toLowerCase().includes(query),
      );
    }
            ...
            <AnimatePresence mode="popLayout">
              {pagedTours.map((tour, index) => (
                <motion.div
                  ...
                  className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col md:flex-row hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
                >
                  {/* Ảnh bên trái */}
                  ...
                  <div className="flex-1 p-5 md:p-6 flex flex-col justify-between">
                    ...
                    <div className="flex items-end justify-between mt-4">
                      ...
                      <Link
                        to={`/tours/${tour.id}`}
                        className="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-2.5 rounded-xl font-bold transition-all shadow-md hover:shadow-lg"
                      >
                        Xem chi tiết
                      </Link>
                    </div>
