...
export default function BookingTour() {
    ...
    fetch(`${import.meta.env.VITE_API_BASE ? import.meta.env.VITE_API_BASE + '/api' : 'http://127.0.0.1:8000/api'}/tours/${tourId}`)
      ...
      .then((data) => {
        ...
        const dep = data.departures.find(
          (d: Departure) => String(d.id) === String(departureId),
        );
        setDeparture(dep);
      });
  }, [tourId, departureId]);
  ...
  const handleBooking = async () => {
    ...
    setLoading(true);
    try {
      // Tính check-out = departure_date + days ngày
      // Ví dụ: tour 2 ngày, khởi hành 2/6 → về 4/6 (2/6 + 2 = 4/6)
      const checkIn = departure.departure_date;
      ...
      if (res.ok) {
        const realTourId = tour?.id?.toString() || tourId || "";
