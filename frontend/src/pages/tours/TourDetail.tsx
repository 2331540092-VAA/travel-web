...
export default function TourDetail() {
  ...
  const groupedSchedules = useMemo(() => {
    ...
    const groups = tour.schedules.reduce((acc: Record<number, DayGroup>, item: Activity) => {
      ...
      return acc;
    }, {} as Record<number, DayGroup>);

    return Object.values(groups).sort(
      (a: DayGroup, b: DayGroup) => a.day_number - b.day_number,
    );
  }, [tour]);
  ...
  const selectedDeparture: Departure | null =
    sortedDepartures.find((d) => d.id === selectedDepartureId) ??
    sortedDepartures[0] ??
    null;
          ...
          <div className="lg:w-2/3 space-y-10">
            ...
            <section className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
              ...
              <div className="relative border-l-2 border-slate-100 ml-6 space-y-12">
                {groupedSchedules.map((group: DayGroup) => {
                    ...
                    <div key={group.day_number} className="relative pl-10">
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {group.activities.map((act: Activity, idx: number) => (
                            <h3
                              key={act.id}
                      ...
                      >
                        <div className="space-y-4">
                          {group.activities.map((act: Activity) => (
                            <div
                              key={act.id}
