      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <button
            ...
            className="border border-sky-500 text-sky-600 hover:bg-sky-50 px-5 py-2 rounded-full text-sm font-semibold transition"
          >
            Quay lại
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {locations.map((location) => (
              <LocationCard key={location.id} location={location} showHeart={true} />
            ))}
          </div>
        )}
      </div>
