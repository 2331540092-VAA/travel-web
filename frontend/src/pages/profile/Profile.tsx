  ...
  useEffect(() => {
    ...
    apiGet<UserProfile>(`/users/${id}`)
      ...
      .catch((err) => {
        ...
        // Dùng dữ liệu từ localStorage làm fallback thay vì trắng trang
        try {
          const fallback: UserProfile = JSON.parse(stored);
          setUser(fallback);
          setForm({
            name: fallback.name || "",
            phone: fallback.phone || "",
            passport_number: fallback.passport_number || "",
            date_of_birth: fallback.date_of_birth
              ? fallback.date_of_birth.split("T")[0]
              : "",
            country_id: fallback.country?.id ? String(fallback.country.id) : "",
            gender: fallback.gender || "",
          });
          setError("Không thể tải dữ liệu mới nhất. Hiển thị thông tin đã lưu.");
        } catch {
          navigate("/login");
        }
      })
  ...
  if (!user) {
    return (
      <div className="flex flex-col justify-center items-center h-screen gap-4">
        <p className="text-gray-500 text-lg">Không thể tải thông tin hồ sơ.</p>
        <button
          onClick={() => navigate("/login")}
          className="bg-sky-500 text-white px-6 py-2 rounded-full font-semibold"
        >
          Đăng nhập lại
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-sky-50 to-white py-10 px-6">
      <div className="max-w-6xl mx-auto">
        {/* Banner lỗi khi dùng dữ liệu cache */}
        {error && (
