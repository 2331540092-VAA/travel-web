...
export default function ServiceReview({ serviceId, type, onReviewSuccess }: ServiceReviewProps) {
  ...
  const loadReviews = async () => {
    try {
      const data = await apiGet<any>(`/reviews?type=${type}&id=${serviceId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errMsg.includes("401")) {
        ...
        setServerError(errMsg || "Không thể gửi đánh giá.");
      }
    } finally {
