...
export default function BookingDetail() {
  ...
  const handlePayment = () => {
    ...
    navigate(
      `/payment?bookingId=${booking.id}&tourId=${booking.target_id}&price=${booking.total_amount || 0
  ...
  const handleCancel = async () => {
    ...
    try {
      const data = await apiPost<any>(`/bookings/${id}/cancel?user_id=${user.id}`, {});
  ...
  return (
    <div className="max-w-2xl mx-auto py-12">
      ...
      <div className="bg-white rounded-lg shadow-lg p-8">
        ...
        <div className="mb-6">
          <span
            className={`px-4 py-2 rounded-full font-semibold ${statusColors[booking.status] || "bg-gray-100 text-gray-800"
              }`}
          >
        ...
        {booking.status === "paid" ? (
          <div className="mb-8 border-2 border-dashed border-blue-200 rounded-2xl p-6 bg-gradient-to-br from-blue-50 to-white flex flex-col md:flex-row items-center gap-6 relative overflow-hidden shadow-sm">
            ...
            <div className="flex-1 w-full space-y-4">
              <div className="flex justify-between items-center border-b border-blue-100 pb-4">
                <h3 className="text-2xl font-black text-blue-900 uppercase tracking-widest">E-Ticket</h3>
              ...
              <div className="grid grid-cols-2 gap-y-6 gap-x-4">
                <div>
                  ...
                  <p className="text-lg font-bold text-gray-800">{JSON.parse(localStorage.getItem("user") || "{}").name || "Quý khách"}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase font-semibold">Mã Đặt Chỗ</p>
                  <p className="text-lg font-bold text-gray-800">#{booking.id}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase font-semibold">Lịch Trình</p>
                  <p className="font-bold text-gray-800">{booking.check_in ? new Date(booking.check_in).toLocaleDateString("vi-VN") : new Date(booking.booking_date).toLocaleDateString("vi-VN")}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 uppercase font-semibold">Số Lượng</p>
                  <p className="font-bold text-gray-800">{booking.quantity} người</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center p-4 bg-white rounded-xl shadow-md border border-gray-100 min-w-[200px]">
              <QRCodeCanvas 
                value={JSON.stringify({ booking_id: booking.id, target: booking.target_id, type: booking.booking_type })}
                size={180}
                bgColor="#ffffff"
                fgColor="#000000"
                level="H"
                includeMargin={true}
              />
              <p className="text-[10px] text-gray-500 mt-3 font-mono tracking-widest">SCAN TO CHECK IN</p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-800">
                   {booking.check_in ? new Date(booking.check_in).toLocaleDateString("vi-VN") : new Date(booking.booking_date).toLocaleDateString("vi-VN")}
                </p>
              </div>
            ...
            <div className="space-y-4">
              <div>
                <p className="text-gray-600 text-sm">Mã tham chiếu (Target ID)</p>
                <p className="text-lg font-semibold text-gray-800">
                  {booking.target_id}
                </p>
              </div>
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  const response = await fetch(`${API_BASE}/bookings/${booking.id}/pdf?user_id=${user.id}`, {
                    method: 'GET',
                  ...
                  await new Promise<void>((resolve, reject) => {
                    ...
                    reader.onerror = () => reject(new Error("Lỗi đọc file (FileReader)"));
                    reader.readAsDataURL(blob);
                  });
                  
                  toast.success("Tải hóa đơn thành công!", { id: "pdf" });
                } catch (error) {
                  toast.error("Lỗi hệ thống khi tải PDF. Vui lòng thử lại sau.", { id: "pdf" });
                }
              }}
