        ...
        {status === "error" && (
          <div className="bg-white rounded-lg shadow-lg p-12 text-center">
            ...
            <div className="flex gap-4 justify-center">
              <button
                onClick={() => {
                  const retryParams = sessionStorage.getItem("paymentRetryParams");
                  const retryUrl = retryParams ? `/payment?${retryParams}` : "/bookings";
                  if (window.location.hostname.includes("ngrok")) {
                    window.location.href = `http://localhost:5173${retryUrl}`;
                  } else {
                    navigate(retryUrl);
                  }
                }}
