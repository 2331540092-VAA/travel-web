...
export default function ExploreSection() {
  ...
  return (
    <section className="mb-20">
      <h2 className="text-2xl font-bold mb-6 text-center text-red-600">Ưu đãi tour sốc nhất</h2>
      {loading ? (
        <div className="text-center py-10">Đang tải ưu đãi...</div>
      ) : tours.length === 0 ? (
        <div className="text-center py-10 text-gray-500">Không có tour ưu đãi nào.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
          {tours.map((tour) => (
            <div key={tour.id} className="flex flex-col bg-white rounded-2xl shadow-lg p-4 h-full">
              <img
                src={tour.image_url}
                alt={tour.name}
              ...
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xl font-bold text-red-600">
                  {tour.price && tour.discount_percent ?
                    (tour.price - (tour.price * tour.discount_percent) / 100).toLocaleString("vi-VN") :
              ...
              <div className="flex flex-wrap gap-2 text-gray-600 text-xs mb-3">
                {tour.departure_location && <span>📍 {tour.departure_location}</span>}
