      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">{restaurants.length} nhà hàng</p>
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50">
              {restaurants.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    {r.image_url ? (
                      <img src={r.image_url} alt={r.name} className="w-20 h-14 object-cover rounded-lg" />
                    ) : (
                      <div className="w-20 h-14 bg-gray-100 rounded-lg flex items-center justify-center text-gray-300 text-xs">No img</div>
                    )}
                  </td>
                  <td className="px-5 py-4 text-sm font-semibold text-gray-800">{r.name}</td>
                  <td className="px-5 py-4 text-sm text-gray-500">{r.location?.name || "—"}</td>
                  <td className="px-5 py-4 text-right text-sm font-medium text-gray-700">
                    {r.avg_price ? `${r.avg_price.toLocaleString("vi-VN")} VNĐ` : "—"}
                  </td>
                  <td className="px-5 py-4 text-center">
                    {r.discount_percent ? (
                      <span className="text-[10px] px-2.5 py-1 rounded-full font-bold bg-orange-100 text-orange-600">-{r.discount_percent}%</span>
                    ) : <span className="text-gray-300 text-sm">—</span>}
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex justify-center gap-1.5">
                      <Link to={`/admin/restaurants/${r.id}/tables`}
                        className="p-2 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Bàn">
                        <LayoutGrid size={16} />
                      </Link>
                      <Link to={`/admin/restaurants/edit/${r.id}`}
              ...
              {restaurants.length === 0 && (
                <tr><td colSpan={7} className="px-5 py-12 text-center text-gray-400 text-sm">Chưa có nhà hàng nào.</td></tr>
              )}
            </tbody>
          </table>
        </div>
